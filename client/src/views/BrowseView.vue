...
const section = computed(() =>
  isMyList.value
    ? undefined
    : catalogStore.sections[sectionKey.value as CatalogSectionKey],
);
...
const rows = computed(() => {
  ...
  if (recentTitles.value.length) {
    extraRows.push({
      id: "recent",
      label: "Assistidos recentemente",
      items: recentTitles.value,
    });
  }
...
const heroPaused = computed(
  () => searching.value || detailQuery.value !== null,
);
const { hero, heroVisible } = useHeroRotation(featured, heroPaused);

function parseTitleKey(
  raw: unknown,
...
function openTitle(item: TitleSummary): void {
  void router.push({
    query: { ...route.query, title: `${item.mediaType}-${item.tmdbId}` },
  });
}
...
<template>
  <div class="min-h-screen animate-fade-in bg-background">
    ...
    <div
      v-if="searching"
      class="min-h-[60vh] animate-fade-in px-5 pt-10 pb-15 sm:px-14"
    >
      ...
      <template v-else>
        ...
        <p
          v-if="!catalogStore.searchResults.length"
          class="text-[15px] text-foreground/45"
        >
    ...
    <template v-else>
      ...
      >
        <div
          class="size-4 rotate-45 rounded-[4px] bg-primary shadow-[0_0_24px_rgba(255,106,0,0.8)]"
        />
        <h2 class="m-0 font-display text-2xl font-bold">
          Catálogo indisponível
        </h2>
        <p
          class="m-0 max-w-[480px] text-[15px] leading-relaxed text-foreground/60"
        >
          {{ catalogStore.error }}
        </p>
      </div>
      ...
      <LoadingSpinner
        v-else-if="catalogStore.loading && !section && !isMyList"
      />
      ...
      <div v-else class="relative z-2 pt-2 pb-20">
        ...
        />
        <div
          v-if="isMyList && !rows.length"
          class="px-5 py-5 text-[14.5px] text-foreground/45 sm:px-14"
        >
